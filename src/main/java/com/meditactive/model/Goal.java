/* mi serve per rappresentare in memoria un obiettivo letto dal file obiettivi.csv */
...
public record Goal(
    ...
    public static Goal fromCsv(String[] columns) {
        // valida il numero minimo di colonne richieste dal csv obiettivi
        if (columns.length < 6) {
            throw new IllegalArgumentException("Riga obiettivi non valida: colonne insufficienti");
        }

        // converte il campo Disponibile in boolean in modo uniforme in tutta l'app
        boolean isAvailable = "SI".equalsIgnoreCase(columns[5].trim());

        // crea l'oggetto Goal con i dati convertiti dal csv
        return new Goal(
    ...
    public Goal withDisponibile(boolean nuovoValore) {
        // valore disponibile senza mutare direttamente l'oggetto record
